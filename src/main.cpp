...
#if defined(NDEBUG)
# error "Paniccoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa36ec73a534f966e32f84b36dd9c6f1e40d175fcd8b166959e6b166afebc9a55");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Paniccoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Paniccoin Signed Message:\n";
...
{
    // Paniccoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1 * 60 * 60; // Paniccoin: 1 hours
static const int64 nTargetSpacing = 2 * 60; // Paniccoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Paniccoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Paniccoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa36ec73a534f966e32f84b36dd9c6f1e40d175fcd8b166959e6b166afebc9a55");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "genesis 22.06.2017";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("021484710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1498149163;
        ...
        assert(block.hashMerkleRoot == uint256("0xb9d4159972c766ae20bcbf82f35dc45958fd6cef62d9ece520dcc16a95f24e9f"));
...
unsigned char pchMessageStart[4] = { 0xab, 0xc0, 0xa6, 0xdb }; // Paniccoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PaniccoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PaniccoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PaniccoinMiner(CWallet *pwallet)
{
    printf("PaniccoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("paniccoin-miner");
    ...
    try { loop {
        ...
        printf("Running PaniccoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PaniccoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PaniccoinMiner, pwallet));
}
